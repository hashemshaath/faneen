...
export const CategoriesSection = () => {
  ...
  return (
    <section id="categories" className="py-12 sm:py-24 bg-background overflow-hidden">
      <div className="container px-4 sm:px-6">
        <div ref={titleRef} className="text-center mb-8 sm:mb-16 will-change-transform">
          <span className="text-xs sm:text-sm font-body text-accent font-semibold tracking-wider">{t('categories.label')}</span>
          <h2 className="font-heading font-bold text-xl sm:text-3xl md:text-4xl text-foreground mt-1.5 sm:mt-3">{t('categories.title')}</h2>
          <p className="font-body text-muted-foreground mt-2 sm:mt-4 max-w-xl mx-auto text-xs sm:text-base">{t('categories.desc')}</p>
        </div>
        <div ref={visRef} className="grid grid-cols-2 lg:grid-cols-3 gap-2.5 sm:gap-6">
          {categories.map((cat, i) => (
            <div
              key={cat.titleKey}
              className={`group relative p-4 sm:p-8 rounded-xl sm:rounded-2xl bg-card dark:bg-card/80 border border-border dark:border-border/40 hover:border-accent/40 active:scale-[0.97] sm:hover:-translate-y-1.5 transition-all duration-500 cursor-pointer ${isVisible ? 'animate-card-slide-up' : 'opacity-0'}`}
            ...
            >
              ...
              <div className="relative z-10">
                <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-lg sm:rounded-xl bg-gradient-gold flex items-center justify-center mb-2.5 sm:mb-5 group-hover:scale-110 transition-transform duration-300">
                  <cat.icon className="w-5 h-5 sm:w-7 sm:h-7 text-secondary-foreground" />
                </div>
                <h3 className="font-heading font-bold text-sm sm:text-xl text-card-foreground mb-1 sm:mb-2 transition-colors duration-300 group-hover:text-accent">{t(cat.titleKey)}</h3>
                <p className="font-body text-muted-foreground text-[11px] sm:text-sm line-clamp-2">{t(cat.descKey)}</p>
              </div>
            </div>
          ))}
        </div>
