...
export const MembershipSection = () => {
  ...
  return (
    <section className="py-14 sm:py-28 bg-background overflow-hidden relative">
      ...
      <div className="container px-4 sm:px-6 relative">
        <div className="text-center mb-10 sm:mb-16">
          ...
          <h2 className="font-heading font-bold text-2xl sm:text-3xl md:text-5xl text-foreground">{t('membership.title')}</h2>
        </div>
        <div ref={visRef} className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-8 max-w-5xl mx-auto">
          {memberships.map((plan, idx) => (
            <div
              key={plan.titleKey}
              className={`group relative p-6 sm:p-8 rounded-2xl sm:rounded-3xl border transition-all duration-500 ${
                plan.featured
                  ...
                  : "bg-card dark:bg-card/60 border-border/50 dark:border-border/30 hover:border-accent/30 active:scale-[0.97] sm:hover:-translate-y-3 hover:shadow-xl hover:shadow-accent/5"
            ...
            >
              ...
              <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-2xl flex items-center justify-center mb-4 sm:mb-5 ${
                ...
                <plan.icon className={`w-6 h-6 sm:w-7 sm:h-7 ${plan.featured ? 'text-gold' : 'text-accent group-hover:text-secondary-foreground'} transition-colors`} />
              </div>

              <h3 className={`font-heading font-bold text-xl sm:text-2xl mb-1.5 sm:mb-2 ${plan.featured ? "text-surface-nav-foreground" : "text-foreground group-hover:text-accent"} transition-colors duration-300`}>{t(plan.titleKey)}</h3>
              <p className={`font-body text-xs sm:text-sm mb-5 sm:mb-7 ${plan.featured ? "text-surface-nav-foreground/60" : "text-muted-foreground"}`}>{t(plan.descKey)}</p>

              <ul className="space-y-2.5 sm:space-y-3 mb-6 sm:mb-8">
                {plan.features.map((f, fi) => (
                  <li
                    key={f.en}
                    className={`flex items-center gap-2.5 font-body text-xs sm:text-sm ${plan.featured ? "text-surface-nav-foreground/80" : "text-muted-foreground group-hover:text-foreground"} transition-all duration-300`}
                    style={{ transitionDelay: `${fi * 50}ms` }}
                  >
                    <div className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 ${plan.featured ? 'bg-gold/20' : 'bg-accent/10'}`}>
              ...
              <Link to="/auth">
                <Button variant={plan.featured ? "hero" : "outline"} className={`w-full transition-all duration-300 active:scale-95 ${!plan.featured ? "group-hover:bg-accent group-hover:text-accent-foreground group-hover:border-accent" : "shadow-lg shadow-gold/20"}`}>
                  {t('membership.start')}
                </Button>
              </Link>
            </div>
