          ...
          {isLoading ? (
            ...
            posts.map((post, i: number) => (
              <Link key={post.id} to={`/blog/${post.slug}`} className="group block h-full">
                <div
                  className={`card-ds card-media transition-all duration-500 hover:shadow-xl hover:shadow-primary/5 hover:-translate-y-1.5 sm:hover:-translate-y-2.5 hover:border-primary/40 h-full flex flex-col ${isVisible ? 'animate-card-slide-up' : ''}`}
                  ...
                  <div className="aspect-video bg-muted relative overflow-hidden">
                    {post.cover_image_url ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-muted to-muted/50">
                        <BookOpen className="w-10 h-10 text-muted-foreground/20 transition-transform duration-500 group-hover:scale-110 group-hover:text-primary/30" />
                      </div>
                    )}
                    ...
                    <span className="absolute top-3 start-3 text-[10px] sm:text-xs font-bold text-primary-foreground bg-primary px-2.5 py-1 rounded-full shadow-lg">
                      {post.category}
                    </span>
                  </div>
                  <div className="card-pad-md space-y-2.5 sm:space-y-3 flex-1 flex flex-col">
                    <h3 className="font-heading font-bold text-sm sm:text-base line-clamp-2 transition-colors duration-300 group-hover:text-primary">
                      {language === 'ar' ? post.title_ar : (post.title_en || post.title_ar)}
                    </h3>
                    {(post.excerpt_ar || post.excerpt_en) && (
                      <p className="text-xs sm:text-sm text-muted-foreground line-clamp-2 flex-1 leading-relaxed">
                        {language === 'ar' ? post.excerpt_ar : (post.excerpt_en || post.excerpt_ar)}
                      </p>
                    )}
                    <div className="flex items-center justify-between pt-3 border-t border-border/30 transition-colors duration-300 group-hover:border-primary/20">
                      {post.published_at && (
