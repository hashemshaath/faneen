        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-7">
          {isLoading ? (
            ...
            projects.map((p: any, i: number) => (
              <Link key={p.id} to={`/projects/${p.id}`} className={`group rounded-2xl overflow-hidden border border-border hover:border-gold/40 bg-card transition-all duration-500 hover:shadow-xl hover:shadow-gold/5 sm:hover:-translate-y-2 block ${isVisible ? 'animate-fade-in' : ''}`} style={{ animationDelay: `${i * 100}ms`, animationFillMode: 'both' }}>
                <div className="aspect-video bg-muted relative overflow-hidden">
                  ...
                  {p.is_featured && (
                    <span className="absolute top-3 start-3 bg-gold text-secondary-foreground text-[10px] px-2.5 py-0.5 rounded-full font-bold shadow-md">
                      {isRTL ? 'مميز' : 'Featured'}
                    </span>
                  )}
                </div>
                <div className="p-4 sm:p-5 space-y-2.5">
                  <h3 className="font-heading font-bold text-base sm:text-lg line-clamp-1 group-hover:text-gold transition-colors">
                  ...
                  <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
                    {p.project_cost && (
                      <span className="flex items-center gap-1 bg-muted dark:bg-muted/50 px-2.5 py-1 rounded-full">
