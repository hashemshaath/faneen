...
import {
  Activity, UserCheck, FileText, CheckCircle2, MessageSquare, Bell, Database,
} from 'lucide-react';
import { Card } from '@/components/ui/card';
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from '@/components/ui/tooltip';
import { pickBi } from '@/components/common/Bilingual';

type DimKey = 'identity' | 'activity' | 'delivery' | 'engagement' | 'inbox';
...
const META: Record<DimKey, {
  ...
  identity: {
    ar: 'هويتك', en: 'Your identity', icon: UserCheck,
    formulaAr: 'اكتمال الاسم والمعرّف العام (رقم العميل).',
    formulaEn: 'Full name + public reference ID completeness.',
  },
  activity: {
    ar: 'نشاط الشراء', en: 'Procurement activity', icon: FileText,
    formulaAr: 'يبلغ ١٠٠ عند ≥٣ عقود مُنشأة.',
    formulaEn: 'Full at ≥3 contracts created.',
  },
  delivery: {
    ar: 'الإنجاز', en: 'Delivery rate', icon: CheckCircle2,
    formulaAr: 'العقود المكتملة ÷ إجمالي العقود × ١٠٠.',
    formulaEn: 'completed / total contracts × 100.',
  },
  engagement: {
    ar: 'التواصل', en: 'Engagement', icon: MessageSquare,
    formulaAr: 'يبلغ ١٠٠ عند ≥٣ محادثات نشطة.',
    formulaEn: 'Full at ≥3 active conversations.',
  },
  inbox: {
    ar: 'متابعة الإشعارات', en: 'Inbox hygiene', icon: Bell,
    formulaAr: '١٠٠ بدون متراكم، يقل تدريجيًا مع كل إشعار غير مقروء (٠ عند ≥١٠).',
    formulaEn: '100 with zero backlog, decays per unread (0 at ≥10).',
  },
};
...
function computeClientHealth(i: ClientHealthInput): {
  score: number;
  grade: keyof typeof GRADE_TONE;
  dims: Dimension[];
} {
  const identityCount = (i.hasFullName ? 1 : 0) + (i.hasRefId ? 1 : 0);
  const identity = Math.round((identityCount / 2) * 100);
  const activity = Math.min(100, Math.round((Math.min(i.totalContracts, 3) / 3) * 100));
  const delivery = i.totalContracts
    ? Math.round((i.completedContracts / i.totalContracts) * 100)
    : 0;
  const engagement = Math.min(100, Math.round((Math.min(i.conversations, 3) / 3) * 100));
  const inbox = Math.max(0, 100 - Math.min(i.unreadNotifications, 10) * 10);

  const dims: Dimension[] = [
    { key: 'identity',   score: identity,   weight: 0.15, numerator: identityCount,        denominator: 2,                source: 'profiles.full_name + ref_id' },
    { key: 'activity',   score: activity,   weight: 0.25, numerator: i.totalContracts,     denominator: 3,                source: 'contracts (client)' },
    { key: 'delivery',   score: delivery,   weight: 0.25, numerator: i.completedContracts, denominator: i.totalContracts, source: "contracts.status='completed'" },
    { key: 'engagement', score: engagement, weight: 0.20, numerator: i.conversations,      denominator: 3,                source: 'messaging.conversations' },
    { key: 'inbox',      score: inbox,      weight: 0.15, numerator: i.unreadNotifications, denominator: 10,              source: 'notifications.is_read = false' },
  ];
...
/**
 * ClientHealthScoreCard — composite score of a client's account
 * health: identity completion, procurement activity, delivery rate,
 * engagement with providers, and inbox hygiene. All values derive
 * from already-fetched query results — no extra reads.
 */
export const ClientHealthScoreCard: React.FC<Props> = React.memo(({ input, isRTL }) => {
  const { score, grade, dims } = React.useMemo(() => computeClientHealth(input), [input]);
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-foreground">
                  {pickBi(isRTL, 'مؤشر صحّة حسابك', 'Your account health score')}
                </h3>
              </div>
...
ClientHealthScoreCard.displayName = 'ClientHealthScoreCard';
