...
const breadcrumbMap: Record<string, { ar: string; en: string }> = {
  ...
  '/admin/categories': { ar: 'مركز التصنيفات', en: 'Taxonomy Center' },
  '/admin/tags': { ar: 'مركز التصنيفات', en: 'Taxonomy Center' },
...
const DashboardLayoutShell: React.FC<DashboardLayoutProps> = ({ children }) => {
  ...
  // Breadcrumbs are rendered globally by <WorkspaceHeader /> via
  // useBreadcrumbs(). Do NOT render a second admin-only breadcrumb here —
  // it caused a duplicate trail (e.g. "Admin / Operations" appearing twice)
  // on /admin/* pages.
