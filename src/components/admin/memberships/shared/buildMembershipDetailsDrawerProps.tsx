...
export function buildProviderSubscriptionDrawerProps(
  ...
  if (typeof input.leadCreditsBalance === 'number') {
    meta.push({
      ...
      value: <span className="tech-content">{input.leadCreditsBalance}</span>,
    });
  }
...
export function buildRejectionDrawerProps(
  ...
  if (input.attemptedBusinessRefId) {
    meta.push({
      ...
      value: <span className="tech-content">{input.attemptedBusinessRefId}</span>,
    });
  }
  if (input.actualBusinessRefId) {
    meta.push({
      label: isRTL ? 'المنشأة (الفعلية)' : 'Business (actual)',
      value: <span className="tech-content">{input.actualBusinessRefId}</span>,
    });
  }
  if (input.userRefId || input.userId) {
    meta.push({
      label: isRTL ? 'المستخدم' : 'User',
      value: <span className="tech-content">{input.userRefId ?? input.userId}</span>,
    });
  }
