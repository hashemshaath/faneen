...
interface Props {
  ...
  leadRefId?: string | null;
  className?: string;
}

/**
 * BUSINESS-CORE-10 — Manual "Create work order from lead request" affordance.
 * Inline card (no popup). On success, renders a quick link to the new WO.
 */
export function CreateWorkOrderFromLeadButton({
  leadRequestId, businessId, defaultTitle, leadRefId, className,
}: Props) {
  ...
  const submit = async () => {
    setSubmitting(true);
    const { data, error } = await createWorkOrderFromLead({
      leadRequestId,
      businessId,
    ...
    <Card className={className}>
      <CardHeader className="p-4 pb-2">
        <CardTitle className="text-sm">
          {isRTL ? "إنشاء أمر عمل من الطلب" : "Create work order from lead"}
          {leadRefId && (
      ...
      <CardContent className="p-4 pt-2 space-y-3">
        <div className="space-y-1">
          <Label htmlFor="wo-lead-title" className="text-xs">
            {isRTL ? "العنوان" : "Title"}
          </Label>
          <Input
            id="wo-lead-title"
        ...
        <div className="space-y-1">
          <Label htmlFor="wo-lead-desc" className="text-xs">
...
export default CreateWorkOrderFromLeadButton;