...
/**
 * Phase 5C-4 guard — the next batch of high-density dashboard files must use
 * the sanctioned bilingual primitives (`pickBi`, `useBi`, `<Bi>`) instead of
 * inline `isRTL ? 'ar' : 'en'` ternaries, must not call
 * `toLocaleString('ar...')`, and must not contain Arabic-Indic digit
 * literals (numbers/dates go through `@/lib/format`).
 */
const FILES = [
  'src/pages/dashboard/DashboardProfileSystems.tsx',
...
describe('Phase 5C-4 — Dashboard profileSystems/portfolio/projects/brands/analytics bilingual hygiene', () => {
