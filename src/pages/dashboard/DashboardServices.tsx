...
import {
  ...
  AlertCircle, Clock, Zap, Download, BarChart3, Hash,
  Layers, Shield, FileText,
} from 'lucide-react';
...
type ActiveSection = 'services' | 'catalog';

/* ─── Price formatter ─── */
const formatPrice = (from: number | null, to: number | null, currency: string) => {
  const f = from ? Number(from).toLocaleString() : '';
  const t = to ? Number(to).toLocaleString() : '';
  if (f && t) return `${f} - ${t} ${currency}`;
  if (f) return `${isRTL ? 'من' : 'from'} ${f} ${currency}`;
  if (t) return `${isRTL ? 'حتى' : 'up to'} ${t} ${currency}`;
  return '';
};
let isRTL = false; // will be set in component
...
const SortableServiceRow = React.memo(({
  ...
  const daysSinceCreated = Math.floor((Date.now() - new Date(s.created_at).getTime()) / 86400000);
  ...
  if (viewMode === 'grid') {
    return (
      <div ref={setNodeRef} style={style} className="h-full">
        <div className={`relative rounded-xl border bg-card p-4 h-full flex flex-col gap-3 group transition-all duration-200 hover:shadow-lg hover:border-primary/30 ${!s.is_active ? 'opacity-50 grayscale-[30%]' : ''} ${isSelected ? 'ring-2 ring-primary border-primary/40' : 'border-border/50'}`}>
          {/* Top row */}
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-2.5">
              <button onClick={() => onSelect(s.id)} className={`w-[18px] h-[18px] rounded border-[1.5px] transition-all flex items-center justify-center shrink-0 ${isSelected ? 'bg-primary border-primary' : 'border-muted-foreground/25 hover:border-primary/60'}`}>
                {isSelected && <CheckCircle2 className="w-3 h-3 text-primary-foreground" />}
              </button>
              <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${s.is_active ? 'bg-primary/10' : 'bg-muted'}`}>
                <Wrench className={`w-4 h-4 ${s.is_active ? 'text-primary' : 'text-muted-foreground'}`} />
              </div>
            </div>
            <div className="flex items-center gap-1">
              {isNew && <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded-full bg-accent text-accent-foreground">{rtl ? 'جديد' : 'New'}</span>}
              {isIncomplete && <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-destructive/10 text-destructive">!</span>}
              <button {...attributes} {...listeners} className="cursor-grab active:cursor-grabbing p-0.5 text-muted-foreground/40 hover:text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <div className="flex-1 min-w-0">
            ...
            {desc && <p className="text-[11px] text-muted-foreground line-clamp-2 mt-1 leading-relaxed">{desc}</p>}
          </div>
          ...
          <div className="flex items-center justify-between pt-2 border-t border-border/40">
            {hasPrice ? (
              <span className="text-xs font-bold text-primary">
                {s.price_from ? Number(s.price_from).toLocaleString() : ''}{s.price_from && s.price_to ? ' – ' : ''}{s.price_to ? Number(s.price_to).toLocaleString() : ''} <span className="text-[10px] font-normal text-muted-foreground">{s.currency_code}</span>
              </span>
            ) : (
              <span className="text-[10px] text-muted-foreground italic">{rtl ? 'بدون تسعير' : 'No price set'}</span>
            )}
            <span className={`text-[9px] font-medium px-1.5 py-0.5 rounded-full ${s.is_active ? 'bg-emerald-500/10 text-emerald-600' : 'bg-muted text-muted-foreground'}`}>
          ...
          <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <Button variant="ghost" size="icon" className="h-7 w-7 rounded-lg text-destructive/70 hover:text-destructive hover:bg-destructive/10" onClick={() => onDelete(s.id)}><Trash2 className="w-3.5 h-3.5" /></Button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div ref={setNodeRef} style={style}>
      <div className={`flex items-center gap-3 p-3 sm:p-3.5 rounded-xl border bg-card transition-all duration-200 group hover:shadow-sm hover:border-primary/20 ${!s.is_active ? 'opacity-50 grayscale-[30%]' : ''} ${isSelected ? 'ring-2 ring-primary border-primary/40' : 'border-border/50'}`}>
        <button onClick={() => onSelect(s.id)} className={`w-[18px] h-[18px] rounded border-[1.5px] transition-all flex items-center justify-center shrink-0 ${isSelected ? 'bg-primary border-primary' : 'border-muted-foreground/25 hover:border-primary/60'}`}>
          {isSelected && <CheckCircle2 className="w-3 h-3 text-primary-foreground" />}
        </button>
        ...
        <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${s.is_active ? 'bg-primary/8' : 'bg-muted'}`}>
          <Wrench className={`w-4 h-4 ${s.is_active ? 'text-primary' : 'text-muted-foreground'}`} />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {isIncomplete && (
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger><AlertCircle className="w-3 h-3 text-destructive/60" /></TooltipTrigger>
                  <TooltipContent><p className="text-xs">{rtl ? 'بيانات ناقصة' : 'Incomplete data'}</p></TooltipContent>
                </Tooltip>
              </TooltipProvider>
            )}
          </div>
          <div className="flex items-center gap-2 mt-0.5">
            {desc && <p className="text-[11px] text-muted-foreground truncate max-w-[200px]">{desc}</p>}
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <span className={`text-[9px] font-medium px-1.5 py-0.5 rounded-full whitespace-nowrap ${s.is_active ? 'bg-emerald-500/10 text-emerald-600' : 'bg-muted text-muted-foreground'}`}>
            {s.is_active ? (rtl ? 'نشط' : 'Active') : (rtl ? 'معطل' : 'Off')}
          </span>
        </div>
...
SortableServiceRow.displayName = 'SortableServiceRow';
...
const DashboardServices = () => {
  ...
  const stats = useMemo(() => {
    ...
    const avgPrice = services.reduce((sum, s) => sum + (Number(s.price_from) || 0), 0) / (total || 1);
    return { total, active, inactive: total - active, noPrice, completeness, avgPrice: Math.round(avgPrice) };
  }, [services]);
  ...
  const saveMut = useMutation({
    ...
    onSuccess: () => {
      ...
      closeForm();
      toast.success(editing ? (rtl ? 'تم تحديث الخدمة' : 'Service updated') : (rtl ? 'تم إضافة الخدمة' : 'Service added'));
    },
  ...
  const deleteMut = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['dashboard-services'] }); setDeleteConfirm(null); toast.success(rtl ? 'تم حذف الخدمة' : 'Service deleted'); },
  });
  ...
  const exportCSV = useCallback(() => {
    const rows = [['Name AR', 'Name EN', 'Description AR', 'Price From', 'Price To', 'Currency', 'Active', 'Created'].join(','),
      ...services.map(s => [
        `"${s.name_ar}"`, `"${s.name_en || ''}"`, `"${(s.description_ar || '').replace(/"/g, '""')}"`,
        s.price_from || '', s.price_to || '', s.currency_code, s.is_active, s.created_at.slice(0, 10),
      ].join(','))
    ].join('\n');
      ...
      <div className="space-y-5">
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">{rtl ? `${stats.total} خدمة · ${stats.active} نشطة` : `${stats.total} services · ${stats.active} active`}</p>
              </div>
            </div>
            <div className="flex gap-2">
              {services.length > 0 && (
                <Button variant="outline" size="sm" className="h-8 text-xs" onClick={exportCSV}>
                  <Download className="w-3.5 h-3.5 me-1" />{rtl ? 'تصدير' : 'Export'}
                </Button>
              )}
              <Button variant="hero" size="sm" className="h-8 text-xs" onClick={() => { closeForm(); setShowForm(true); setActiveSection('services'); scrollToForm(); }}>
                <Plus className="w-3.5 h-3.5 me-1" />{rtl ? 'إضافة خدمة' : 'Add Service'}
              </Button>
            </div>
          </div>
          ...
          <div className="flex gap-1 p-1 bg-muted/50 rounded-xl w-fit border border-border/40">
            {([
              { key: 'services' as const, label: rtl ? 'خدماتي' : 'My Services', icon: Package, count: stats.total },
              { key: 'catalog' as const, label: rtl ? 'الكتالوج' : 'Catalog', icon: Sparkles, count: serviceCatalog.reduce((a, g) => a + g.services.length, 0) },
            ]).map(tab => (
              <button key={tab.key} onClick={() => { setActiveSection(tab.key); setShowForm(false); }}
                className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-xs font-medium transition-all ${activeSection === tab.key ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>
                <tab.icon className="w-3.5 h-3.5" />
                {tab.label}
                <span className={`text-[10px] px-1 py-px rounded ${activeSection === tab.key ? 'bg-primary/10 text-primary' : 'bg-muted-foreground/10'}`}>{tab.count}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        {/* ═══ Stats Row ═══ */}
        {services.length > 0 && activeSection === 'services' && (
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-2">
            {[
              { l: rtl ? 'إجمالي' : 'Total', v: stats.total, icon: Package, cls: 'text-primary bg-primary/10' },
              { l: rtl ? 'نشطة' : 'Active', v: stats.active, icon: CheckCircle2, cls: 'text-emerald-600 bg-emerald-500/10' },
        ...
        {/* Completeness Alert */}
        {stats.completeness < 100 && stats.total > 0 && activeSection === 'services' && (
          <div className="flex items-center gap-3 p-3 rounded-xl bg-accent/50 border border-accent">
            <Zap className="w-4 h-4 text-primary shrink-0" />
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between mb-1">
                <span className="text-[11px] font-medium">{rtl ? 'اكتمال بيانات الخدمات' : 'Service data completeness'}</span>
                <span className="text-[11px] font-bold text-primary">{stats.completeness}%</span>
              </div>
              <Progress value={stats.completeness} className="h-1" />
            </div>
            <Button variant="ghost" size="sm" className="h-6 text-[10px] shrink-0" onClick={() => { setFilterMode('no-price'); setActiveSection('services'); }}>
          ...
          <>
                  ...
                  <CardContent className="space-y-4 pb-5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        ...
                        <Input value={form.name_ar} onChange={e => setForm(p => ({ ...p, name_ar: e.target.value }))} placeholder={rtl ? 'مثال: تركيب نوافذ ألمنيوم' : 'e.g. Aluminum window installation'} className="h-9" />
                      </div>
                      <div className="space-y-1.5">
                        <div className="flex items-center justify-between">
                        ...
                        <Input value={form.name_en} onChange={e => setForm(p => ({ ...p, name_en: e.target.value }))} dir="ltr" placeholder="e.g. Aluminum window installation" className="h-9" />
                      </div>
                    </div>
                    {/* Descriptions */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        ...
                        <Textarea value={form.description_ar} onChange={e => setForm(p => ({ ...p, description_ar: e.target.value }))} rows={2} placeholder={rtl ? 'وصف مختصر...' : 'Brief description...'} className="resize-none text-sm" />
                      </div>
                      <div className="space-y-1.5">
                        <div className="flex items-center justify-between">
                        ...
                        <Textarea value={form.description_en} onChange={e => setForm(p => ({ ...p, description_en: e.target.value }))} rows={2} dir="ltr" placeholder="Brief description..." className="resize-none text-sm" />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      ...
                      <div className="flex items-end pb-1">
                        <div className="flex items-center gap-2 p-2 rounded-lg bg-muted/50 border border-border/40 w-full">
                    ...
                    <div className="flex gap-2 pt-1">
                      <Button onClick={() => saveMut.mutate()} disabled={!form.name_ar.trim() || saveMut.isPending} variant="hero" className="flex-1 h-9">
                      ...
                      <Button variant="outline" className="h-9" onClick={closeForm}>{rtl ? 'إلغاء' : 'Cancel'}</Button>
                    </div>
                  </CardContent>
                </Card>
              </div>
            ...
            {services.length > 0 && (
              <div className="flex flex-col gap-2.5">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={toggleSelectAll} className="p-1.5 rounded-lg border border-border/40 hover:bg-muted/50 transition-colors" title={rtl ? 'تحديد الكل' : 'Select All'}>
                    <CheckCircle2 className={`w-3.5 h-3.5 ${selectedIds.size === filteredServices.length && filteredServices.length > 0 ? 'text-primary' : 'text-muted-foreground/40'}`} />
                  </button>
                </div>
            ...
            {selectedIds.size > 0 && (
              <div className="flex items-center gap-2 p-2 rounded-xl bg-primary/5 border border-primary/15 animate-in fade-in-0 duration-150">
                ...
                <Button size="sm" variant="outline" className="h-6 text-[10px] px-2" onClick={() => bulkToggleMut.mutate(true)} disabled={bulkToggleMut.isPending}><Eye className="w-3 h-3 me-0.5" />{rtl ? 'تفعيل' : 'On'}</Button>
                <Button size="sm" variant="outline" className="h-6 text-[10px] px-2" onClick={() => bulkToggleMut.mutate(false)} disabled={bulkToggleMut.isPending}><EyeOff className="w-3 h-3 me-0.5" />{rtl ? 'تعطيل' : 'Off'}</Button>
                <Button size="sm" variant="outline" className="h-6 text-[10px] px-2 text-destructive hover:text-destructive" onClick={() => bulkDeleteMut.mutate()} disabled={bulkDeleteMut.isPending}><Trash2 className="w-3 h-3 me-0.5" />{rtl ? 'حذف' : 'Del'}</Button>
            ...
            {deleteConfirm && (
              <div className="flex items-center gap-3 p-3 rounded-xl border border-destructive/30 bg-destructive/5 animate-in fade-in-0 slide-in-from-top-1 duration-150">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[11px] text-muted-foreground">{rtl ? 'لا يمكن التراجع عن هذا الإجراء' : 'This action cannot be undone'}</p>
                </div>
                <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => setDeleteConfirm(null)}>{rtl ? 'إلغاء' : 'Cancel'}</Button>
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col items-center py-16 text-center">
                ...
                <h3 className="text-base font-semibold mb-1">{rtl ? 'لا توجد خدمات بعد' : 'No services yet'}</h3>
                <p className="text-sm text-muted-foreground max-w-xs mb-5">
                  {rtl ? 'أضف خدماتك أو اختر من الكتالوج الجاهز' : 'Add your services or pick from the catalog'}
                </p>
                <div className="flex gap-2">
                  <Button variant="hero" size="sm" onClick={() => setShowForm(true)}><Plus className="w-4 h-4 me-1" />{rtl ? 'إضافة خدمة' : 'Add Service'}</Button>
                  <Button variant="outline" size="sm" onClick={() => setActiveSection('catalog')}><Sparkles className="w-4 h-4 me-1" />{rtl ? 'تصفح الكتالوج' : 'Browse Catalog'}</Button>
                </div>
              </div>
              ...
              <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                <SortableContext items={filteredServices.map(s => s.id)} strategy={verticalListSortingStrategy}>
                  <div className={viewMode === 'grid' ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5' : 'space-y-1.5'}>
                    {filteredServices.map(s => (
                      <SortableServiceRow
                        key={s.id} s={s} rtl={rtl} viewMode={viewMode}
                        isSelected={selectedIds.has(s.id)}
                        onEdit={openEdit} onToggle={s => toggleMut.mutate(s)}
                        onDuplicate={duplicateService} onDelete={id => setDeleteConfirm(id)}
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[11px] text-muted-foreground shrink-0">
                {serviceCatalog.reduce((a, g) => a + g.services.length, 0)} {rtl ? 'خدمة متاحة' : 'available'}
              </span>
            </div>
            ...
            {filteredCatalog.map((group) => {
              ...
              return (
                <div key={group.id} className="rounded-xl border border-border/50 overflow-hidden bg-card hover:shadow-sm transition-shadow">
                  <button className="w-full flex items-center gap-3 p-3 hover:bg-muted/20 transition-colors text-start" onClick={() => toggleGroup(group.id)}>
                    <span className={`w-9 h-9 rounded-lg flex items-center justify-center text-base ${group.color}`}>{group.icon}</span>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <h3 className="font-semibold text-sm">{rtl ? group.name_ar : group.name_en}</h3>
                        <span className="text-[10px] text-muted-foreground">{group.services.length}</span>
                        {addedCount > 0 && (
                          <span className="text-[10px] font-medium text-emerald-600 bg-emerald-500/10 px-1.5 py-px rounded-full flex items-center gap-0.5">
                            <CheckCircle2 className="w-2.5 h-2.5" />{addedCount}/{group.services.length}
                          </span>
                        )}
                      </div>
                      {addedCount > 0 && addedCount < group.services.length && (
                        <Progress value={progress} className="h-0.5 mt-1.5 max-w-[100px]" />
                      )}
                    </div>
                  ...
                  {isExpanded && (
                    <div className="border-t border-border/40 animate-in fade-in-0 duration-150">
                        ...
                        {group.services.map((item, idx) => {
                          ...
                          return (
                            <button key={idx} disabled={added} onClick={() => quickAddFromCatalog(item)}
                              className={`p-2.5 rounded-lg border text-start transition-all ${added ? 'bg-emerald-500/5 border-emerald-500/15 cursor-default' : 'border-border/40 hover:border-primary/30 hover:bg-primary/5 cursor-pointer'}`}>
                              <div className="flex items-center gap-1.5">
                                {added ? <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500 shrink-0" /> : <Plus className="w-3.5 h-3.5 text-primary shrink-0" />}
                                <span className="font-medium text-xs truncate">{rtl ? item.name_ar : item.name_en}</span>
                              </div>
                              <p className="text-[10px] text-muted-foreground mt-0.5 line-clamp-1 ps-5">{rtl ? item.description_ar : item.description_en}</p>
                            </button>
                          );
                        })}
                      </div>
                      {group.brands && group.brands.length > 0 && (
                        <div className="border-t border-border/40 p-3">
                          <button className="flex items-center gap-1.5 text-[11px] font-medium text-muted-foreground hover:text-foreground transition-colors mb-2"
                            ...
                            {rtl ? 'الماركات' : 'Brands'} ({group.brands.length})
                          ...
                          {showBrands === group.id && (
                            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-1.5 animate-in fade-in-0 duration-150">
                              {group.brands.map((brand, bi) => (
                                <div key={bi} className="flex items-center gap-2 p-2 rounded-lg bg-muted/30 border border-border/30">
                                  <div className={`w-6 h-6 rounded flex items-center justify-center ${brand.type === 'international' ? 'bg-blue-500/10 text-blue-600' : 'bg-emerald-500/10 text-emerald-600'}`}>
