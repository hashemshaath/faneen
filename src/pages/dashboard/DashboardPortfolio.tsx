...
const SortableCard = ({ item, isRTL, onDelete }: { item: PortfolioItem; isRTL: boolean; onDelete: (id: string) => void }) => {
  ...
  return (
    <Card ref={setNodeRef} style={style} className={`overflow-hidden group relative border-border/50 ${isDragging ? 'shadow-xl ring-2 ring-primary' : ''}`}>
      <div className="aspect-square bg-muted relative">
        ...
        <button {...attributes} {...listeners} className="absolute top-2 end-2 bg-background/80 backdrop-blur-sm rounded-md p-1.5 cursor-grab active:cursor-grabbing opacity-0 group-hover:opacity-100 transition-opacity shadow-sm" title={isRTL ? 'اسحب لإعادة الترتيب' : 'Drag to reorder'}>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">{isRTL ? 'أضف صور وفيديوهات أعمالك — اسحب لإعادة الترتيب' : 'Add photos and videos — drag to reorder'}</p>
          </div>
          {!showForm && (
        ...
        {showForm && (
          <Card className="border-accent/30 bg-accent/5">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <CardTitle className="text-base">{isRTL ? 'إضافة عمل جديد' : 'Add New Work'}</CardTitle>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-1">
                ...
                <ImageUpload bucket="portfolio-images" value={form.media_url} onChange={(url) => setForm({ ...form, media_url: url })} onRemove={() => setForm({ ...form, media_url: '' })} aspectRatio="square" placeholder={isRTL ? 'اضغط لرفع صورة' : 'Click to upload image'} />
              </div>
              <div className="space-y-1">
          ...
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
            <SortableContext items={items.map(i => i.id)} strategy={rectSortingStrategy}>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {items.map((item) => (
                  <SortableCard key={item.id} item={item} isRTL={isRTL} onDelete={(id) => deleteMutation.mutate(id)} />
                ))}
              </div>
            </SortableContext>
          </DndContext>
