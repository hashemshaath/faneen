...
import { Plus, Pencil, Trash2, Loader2, FolderTree, Search, X } from 'lucide-react';

interface CategoryForm {
  name_ar: string; name_en: string; slug: string; description_ar: string;
  description_en: string; icon: string; parent_id: string | null; is_active: boolean; sort_order: number;
}
...
const AdminCategories = () => {
  ...
  const filtered = categories.filter(c => !search || c.name_ar.includes(search) || c.name_en.toLowerCase().includes(search.toLowerCase()));
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div>
            <h1 className="font-heading font-bold text-2xl flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-accent/10 flex items-center justify-center"><FolderTree className="w-5 h-5 text-accent" /></div>
              {isRTL ? 'إدارة التصنيفات' : 'Categories Management'}
            </h1>
            <p className="text-muted-foreground text-sm mt-1">{isRTL ? 'إضافة وتعديل التصنيفات الهرمية للمنصة' : 'Add and edit hierarchical categories'}</p>
          </div>
          {!showForm && (
          ...
          <Card className="border-accent/30 bg-accent/5">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <CardTitle className="text-base">{editingId ? (isRTL ? 'تعديل التصنيف' : 'Edit Category') : (isRTL ? 'تصنيف جديد' : 'New Category')}</CardTitle>
              ...
              <div>
                ...
                <Select value={form.parent_id || 'none'} onValueChange={v => setForm(f => ({ ...f, parent_id: v === 'none' ? null : v }))}>
                  ...
                  <SelectContent>
                    ...
                    {parentCategories.filter(c => c.id !== editingId).map(c => <SelectItem key={c.id} value={c.id}>{isRTL ? c.name_ar : c.name_en}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><Label>{isRTL ? 'الترتيب' : 'Sort Order'}</Label><Input type="number" value={form.sort_order} onChange={e => setForm(f => ({ ...f, sort_order: parseInt(e.target.value) || 0 }))} /></div>
                <div className="flex items-center gap-2 pt-6"><Switch checked={form.is_active} onCheckedChange={v => setForm(f => ({ ...f, is_active: v }))} /><Label>{isRTL ? 'مفعّل' : 'Active'}</Label></div>
              </div>
              <div><Label>{isRTL ? 'الوصف (عربي)' : 'Description (Arabic)'}</Label><Textarea value={form.description_ar} onChange={e => setForm(f => ({ ...f, description_ar: e.target.value }))} rows={2} /></div>
              <div><Label>{isRTL ? 'الوصف (إنجليزي)' : 'Description (English)'}</Label><Textarea value={form.description_en} onChange={e => setForm(f => ({ ...f, description_en: e.target.value }))} rows={2} /></div>
                    ...
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-2 text-xs text-muted-foreground">
                          ...
                          {!cat.is_active && <Badge variant="destructive" className="text-[10px]">{isRTL ? 'معطل' : 'Inactive'}</Badge>}
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-1">
                      <Button variant="ghost" size="icon" onClick={() => openEdit(cat)}><Pencil className="w-4 h-4" /></Button>
                      <Button variant="ghost" size="icon" className="text-destructive" onClick={() => setDeletingId(cat.id)}><Trash2 className="w-4 h-4" /></Button>
                    </div>
                  </CardContent>
                </Card>
              );
            })}
            {filtered.length === 0 && <Card><CardContent className="p-12 text-center text-muted-foreground">{isRTL ? 'لا توجد تصنيفات' : 'No categories'}</CardContent></Card>}
          </div>
        )}
