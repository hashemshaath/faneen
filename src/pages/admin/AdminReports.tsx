...
const REPORTS: ReportDef[] = [
  ...
  },
  {
    ...
    async fetch(range) {
      const { data, error } = await supabase
        .from('profiles')
        .select('reference_id, email, display_name, account_type, created_at, country_id')
        .gte('created_at', `${range.from}T00:00:00Z`)
        .lte('created_at', `${range.to}T23:59:59Z`)
        .order('created_at', { ascending: false })
        .limit(5000);
      if (error) throw error;
      const headers = ['reference_id','email','display_name','account_type','created_at','country_id'] as const;
      return { rows: (data ?? []) as Record<string, unknown>[], headers };
    },
  },
  {
    ...
    async fetch(range) {
      const { data, error } = await supabase
        .from('businesses')
        .select('reference_id, name_ar, name_en, verified, is_active, created_at, owner_id')
        .gte('created_at', `${range.from}T00:00:00Z`)
        .lte('created_at', `${range.to}T23:59:59Z`)
        .order('created_at', { ascending: false })
        .limit(5000);
      if (error) throw error;
      const headers = ['reference_id','name_ar','name_en','verified','is_active','created_at','owner_id'] as const;
      return { rows: (data ?? []) as Record<string, unknown>[], headers };
    },
  },
  {
