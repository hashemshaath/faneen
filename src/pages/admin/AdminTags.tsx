...
const emptyForm: TagForm = {
  ...
  icon: '', color: '', is_active: true, sort_order: 0,
};
...
const AdminTags = () => {
  ...
  const openEdit = (tag: any) => {
    ...
    setForm({
      ...
      tag_group: tag.tag_group, icon: tag.icon || '', color: tag.color || '',
  ...
  const groupCounts = tags.reduce((acc, t) => {
    acc[t.tag_group] = (acc[t.tag_group] || 0) + 1;
    return acc;
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">
              {isRTL ? `${tags.length} وسم في ${Object.keys(groupCounts).length} مجموعة` : `${tags.length} tags in ${Object.keys(groupCounts).length} groups`}
            </p>
          </div>
              ...
              <SelectContent>
                ...
                {tagGroups.map(g => (
                  <SelectItem key={g.value} value={g.value}>
                    {isRTL ? g.ar : g.en} ({groupCounts[g.value] || 0})
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
        ...
        {isLoading ? (
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            ...
            {filtered.length === 0 && (
              <Card className="col-span-full"><CardContent className="p-12 text-center text-muted-foreground">
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogContent className="max-w-lg">
            <DialogHeader>
              <DialogTitle>{editingId ? (isRTL ? 'تعديل الوسم' : 'Edit Tag') : (isRTL ? 'وسم جديد' : 'New Tag')}</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <Select value={form.tag_group} onValueChange={v => setForm(f => ({ ...f, tag_group: v }))}>
                    ...
                    <SelectContent>
                      {tagGroups.map(g => (
                        <SelectItem key={g.value} value={g.value}>{isRTL ? g.ar : g.en}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  ...
                  <Input type="color" value={form.color || '#3b82f6'} onChange={e => setForm(f => ({ ...f, color: e.target.value }))} className="h-9" />
                </div>
                <div>
