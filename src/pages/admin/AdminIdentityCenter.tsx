...
const SECTIONS: TokenSection[] = [
  ...
  },
  {
    ...
    tokens: [
      ...
      { key: '--alert-success-bg', labelAr: 'خلفية النجاح (HSL)', labelEn: 'Success bg (HSL)', kind: 'hsl', placeholder: '159 76% 34%' },
      { key: '--alert-warning-bg', labelAr: 'خلفية التحذير (HSL)', labelEn: 'Warning bg (HSL)', kind: 'hsl', placeholder: '33 92% 36%' },
      { key: '--alert-error-bg',   labelAr: 'خلفية الخطأ (HSL)',   labelEn: 'Error bg (HSL)',   kind: 'hsl', placeholder: '0 67% 46%' },
      { key: '--alert-info-bg',    labelAr: 'خلفية المعلومات (HSL)', labelEn: 'Info bg (HSL)',  kind: 'hsl', placeholder: '215 58% 43%' },
    ],
  },
  {
    id: 'button-states',
    titleAr: 'حالات الأزرار',
    titleEn: 'Button States',
    descAr: 'سلوك التحويم والضغط والتعطيل.',
    descEn: 'Hover, active, and disabled behaviour.',
    tokens: [
      { key: '--btn-disabled-opacity', labelAr: 'شفافية المعطّل', labelEn: 'Disabled opacity', kind: 'number', placeholder: '0.5' },
      { key: '--btn-hover-lift',       labelAr: 'رفع عند التحويم', labelEn: 'Hover lift',     kind: 'px',     placeholder: '-1px' },
      { key: '--btn-active-scale',     labelAr: 'تكبير عند الضغط', labelEn: 'Active scale',   kind: 'number', placeholder: '0.98' },
      { key: '--btn-ghost-hover-bg',   labelAr: 'خلفية Ghost عند التحويم (HSL)', labelEn: 'Ghost hover bg (HSL)', kind: 'hsl', placeholder: '220 17% 94%' },
    ],
  },
  {
    id: 'form-states',
    titleAr: 'حالات الحقول',
    titleEn: 'Form States',
    descAr: 'حالات الخطأ والقراءة فقط والـ placeholder.',
    descEn: 'Error, read-only, and placeholder states.',
    tokens: [
      { key: '--field-bg-readonly',   labelAr: 'خلفية للقراءة فقط (HSL)', labelEn: 'Readonly bg (HSL)',   kind: 'hsl', placeholder: '220 17% 96%' },
      { key: '--field-bg-error',      labelAr: 'خلفية الخطأ (HSL)',       labelEn: 'Error bg (HSL)',      kind: 'hsl', placeholder: '0 67% 96%' },
      { key: '--field-border-error',  labelAr: 'إطار الخطأ (HSL)',        labelEn: 'Error border (HSL)',  kind: 'hsl', placeholder: '0 67% 46%' },
      { key: '--field-placeholder',   labelAr: 'لون placeholder (HSL)',   labelEn: 'Placeholder (HSL)',   kind: 'hsl', placeholder: '220 9% 55%' },
    ],
  },
  {
    id: 'cards',
    titleAr: 'البطاقات',
    titleEn: 'Cards',
    descAr: 'حشوات البطاقات وارتفاعها (الظلال).',
    descEn: 'Card padding and elevation (shadows).',
    tokens: [
      { key: '--card-padding-sm', labelAr: 'حشوة صغيرة', labelEn: 'Padding sm', kind: 'px', placeholder: '12px' },
      { key: '--card-padding-md', labelAr: 'حشوة متوسطة', labelEn: 'Padding md', kind: 'px', placeholder: '20px' },
      { key: '--card-padding-lg', labelAr: 'حشوة كبيرة', labelEn: 'Padding lg', kind: 'px', placeholder: '28px' },
    ],
  },
  {
    id: 'motion',
    titleAr: 'الحركة',
    titleEn: 'Motion',
    descAr: 'مدد ومنحنيات الانتقال للواجهة بالكامل.',
    descEn: 'Transition durations and easing curves.',
    tokens: [
      { key: '--dur-fast',        labelAr: 'مدة سريعة',   labelEn: 'Fast duration',   kind: 'text', placeholder: '150ms' },
      { key: '--dur-base',        labelAr: 'مدة أساسية',  labelEn: 'Base duration',   kind: 'text', placeholder: '250ms' },
      { key: '--dur-slow',        labelAr: 'مدة بطيئة',   labelEn: 'Slow duration',   kind: 'text', placeholder: '400ms' },
      { key: '--ease-standard',   labelAr: 'منحنى قياسي', labelEn: 'Standard easing', kind: 'text', placeholder: 'cubic-bezier(0.4, 0, 0.2, 1)' },
      { key: '--ease-emphasized', labelAr: 'منحنى مؤكَّد', labelEn: 'Emphasized easing', kind: 'text', placeholder: 'cubic-bezier(0.22, 0.61, 0.36, 1)' },
    ],
  },
  {
    id: 'shadows',
    titleAr: 'الظلال (الارتفاع)',
    titleEn: 'Shadows (Elevation)',
    descAr: 'أربع مستويات للارتفاع مستخدمة عبر البطاقات والقوائم المنسدلة.',
    descEn: 'Four elevation levels used across cards and dropdowns.',
    tokens: [
      { key: '--elev-1', labelAr: 'مستوى 1 (سكون)',   labelEn: 'Level 1 (resting)', kind: 'text', placeholder: '0 1px 2px hsl(220 35% 15% / 0.04)' },
      { key: '--elev-2', labelAr: 'مستوى 2 (تحويم)',  labelEn: 'Level 2 (hover)',   kind: 'text', placeholder: '0 4px 12px -2px hsl(220 35% 15% / 0.08)' },
      { key: '--elev-3', labelAr: 'مستوى 3 (قائمة)',  labelEn: 'Level 3 (popover)', kind: 'text', placeholder: '0 10px 30px -6px hsl(220 35% 15% / 0.12)' },
      { key: '--elev-4', labelAr: 'مستوى 4 (نافذة)',  labelEn: 'Level 4 (modal)',   kind: 'text', placeholder: '0 24px 60px -12px hsl(220 35% 15% / 0.18)' },
    ],
  },
  {
    id: 'layout',
  ...
  return (
    <DashboardLayout>
      ...
      <AdminPageHeader
        icon={Sparkles}
        tone="primary"
        eyebrow={isRTL ? 'مركز الهوية' : 'Identity Center'}
        title={isRTL ? 'الهوية البصرية الموحّدة' : 'Unified Visual Identity'}
        subtitle={isRTL
