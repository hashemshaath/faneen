...
const Blog = () => {
  ...
  return (
    <div className="min-h-screen bg-background" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      {/* Hero Header */}
      <div ref={heroRef} className="relative bg-primary pt-24 pb-16 overflow-hidden">
        {/* Decorative elements */}
        <div className="absolute inset-0 opacity-10">
          <div className="absolute top-10 start-10 w-72 h-72 bg-accent rounded-full blur-[100px]" />
          <div className="absolute bottom-0 end-20 w-96 h-96 bg-accent rounded-full blur-[120px]" />
        </div>
        <div className={`container relative z-10 transition-all duration-700 ${heroVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <div className="text-center max-w-2xl mx-auto">
            <div className="inline-flex items-center gap-2 bg-accent/20 backdrop-blur-sm rounded-full px-4 py-1.5 mb-4">
              <BookOpen className="w-4 h-4 text-accent" />
              <span className="text-sm font-medium text-accent">{isRTL ? 'مدونة المنصة' : 'Platform Blog'}</span>
            </div>
            ...
            <div className="relative max-w-lg mx-auto">
              ...
              <Input
                ...
                className="ps-12 h-12 rounded-xl bg-background/95 backdrop-blur-md border-border/50 shadow-lg text-foreground placeholder:text-muted-foreground"
              />
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide pb-1">
            {Object.entries(blogCategories).map(([key, cat]) => (
              <button
                ...
                className={`shrink-0 inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 ${
                  activeCategory === key
        ...
        {isLoading ? (
          <div className="flex justify-center py-20">
            <div className="flex flex-col items-center gap-3">
              ...
              <span className="text-sm text-muted-foreground">{isRTL ? 'جارِ التحميل...' : 'Loading...'}</span>
            </div>
          </div>
        ) : posts.length === 0 ? (
          <div className="text-center py-24 text-muted-foreground">
            <FileText className="w-20 h-20 mx-auto mb-4 opacity-20" />
            <p className="text-lg">{isRTL ? 'لا توجد مقالات حالياً' : 'No articles available'}</p>
          </div>
        ) : (
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              {featuredPost && activeCategory === 'all' && !searchQuery && (
                <Link to={`/blog/${featuredPost.slug}`} className="group block">
                  <Card className="overflow-hidden border-border/50 hover:border-accent/40 transition-all duration-500 hover:shadow-2xl hover:shadow-accent/10 hover:-translate-y-1">
                      ...
                      <div className="relative aspect-[2/1] bg-muted overflow-hidden">
                        ...
                        <div className="absolute bottom-0 start-0 end-0 p-6 text-white">
                          <div className="flex items-center gap-2 mb-2">
                            <Badge variant="outline" className="text-[11px] border-white/30 text-white/90 bg-white/10 backdrop-blur-sm">
                              {blogCategories[featuredPost.category]?.[language] || featuredPost.category}
                            </Badge>
                          </div>
                          <h2 className="font-heading font-bold text-xl md:text-2xl mb-2 group-hover:text-accent transition-colors line-clamp-2">
              ...
              <div ref={gridRef} className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                {(activeCategory === 'all' && !searchQuery ? regularPosts : filteredPosts).map((post: any, i: number) => (
                  <Link key={post.id} to={`/blog/${post.slug}`} className="group block">
                    <Card
                      className={`overflow-hidden h-full border-border/50 hover:border-accent/30 transition-all duration-500 hover:shadow-xl hover:shadow-accent/5 hover:-translate-y-2 ${gridVisible ? 'animate-fade-in' : 'opacity-0'}`}
                    ...
                    >
                      <CardContent className="p-0 flex flex-col h-full">
                        <div className="relative aspect-video bg-muted overflow-hidden">
                          {post.cover_image_url ? (
                            ...
                            <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-muted to-muted/50">
                              <BookOpen className="w-10 h-10 text-muted-foreground/15 group-hover:text-accent/30 transition-colors duration-500" />
                            </div>
                          )}
                          ...
                          <div className="absolute bottom-2 end-2 flex items-center gap-1 bg-background/80 backdrop-blur-sm rounded-md px-2 py-1 text-[10px] text-muted-foreground">
                        ...
                        <div className="p-4 flex-1 flex flex-col space-y-2">
                          ...
                          <h3 className="font-heading font-bold text-sm line-clamp-2 group-hover:text-accent transition-colors duration-300">
              ...
              {filteredPosts.length === 0 && !isLoading && (
                <div className="text-center py-16 text-muted-foreground">
                  ...
                  <p className="text-sm">{isRTL ? 'جرّب تغيير كلمات البحث أو التصنيف' : 'Try different search terms or category'}</p>
                </div>
              )}
            </div>
            ...
            <aside className="space-y-6">
              ...
              <Card className="border-border/50">
                <CardContent className="p-5">
                  <h3 className="font-heading font-bold text-base flex items-center gap-2 mb-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {allTags.map((tag) => (
                        <button
                          ...
                          className="px-3 py-1.5 rounded-full bg-muted/60 text-xs font-medium text-muted-foreground hover:bg-accent/10 hover:text-accent transition-colors"
                        >
                          #{tag}
                        </button>
                      ))}
              ...
              <Card className="border-border/50 bg-gradient-to-br from-primary/5 to-accent/5">
                <CardContent className="p-5 text-center space-y-3">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-muted-foreground">{isRTL ? 'مقال منشور' : 'Published Articles'}</div>
                  </div>
