...
const OWNER_BIZ_MIGRATED: Array<{ path: string; key: string }> = [
  ...
  // DashboardServices renamed its query key to disambiguate from other
  // owner-business reads when multiple sibling queries appear on the page.
  { path: 'src/pages/dashboard/DashboardServices.tsx', key: "['my-business-services-page', user?.id, activeOwnerEntityId]" },
  ...
  { path: 'src/pages/dashboard/DashboardContracts.tsx', key: "['my-business-id-contracts', user?.id, activeOwnerEntityId]" },
