...
ALTER TABLE public.client_sites
  ADD COLUMN IF NOT EXISTS municipal_license_no           text,
  ADD COLUMN IF NOT EXISTS municipal_license_issue_date   date,
  ADD COLUMN IF NOT EXISTS municipal_license_expiry_date  date,
  ADD COLUMN IF NOT EXISTS title_deed_no                  text,
  ADD COLUMN IF NOT EXISTS title_deed_date                date,
...
-- Recreate create_client_site to also accept the new government fields.
...
-- Recreate update_client_site adding the new fields.
...
BEGIN
  ...
  IF _patch ? 'short_address' THEN
    ...
    END IF;
  END IF;
  IF _patch ? 'municipal_license_issue_date' THEN
    v_lic_issue := nullif(_patch->>'municipal_license_issue_date','')::date;
  ELSE
    v_lic_issue := v_row.municipal_license_issue_date;
  END IF;
  IF _patch ? 'municipal_license_expiry_date' THEN
    v_lic_expiry := nullif(_patch->>'municipal_license_expiry_date','')::date;
  ELSE
    v_lic_expiry := v_row.municipal_license_expiry_date;
  END IF;
  IF v_lic_issue IS NOT NULL AND v_lic_expiry IS NOT NULL AND v_lic_expiry < v_lic_issue THEN
    RAISE EXCEPTION 'INVALID_LICENSE_DATES' USING ERRCODE = '22023';
  END IF;
  ...
  UPDATE public.client_sites SET
    ...
    municipal_license_no           = CASE WHEN _patch ? 'municipal_license_no'  THEN nullif(btrim(coalesce(_patch->>'municipal_license_no','')),'') ELSE municipal_license_no END,
    municipal_license_issue_date   = v_lic_issue,
    municipal_license_expiry_date  = v_lic_expiry,
    title_deed_no                  = CASE WHEN _patch ? 'title_deed_no'         THEN nullif(btrim(coalesce(_patch->>'title_deed_no','')),'') ELSE title_deed_no END,
    title_deed_date                = CASE WHEN _patch ? 'title_deed_date'       THEN nullif(_patch->>'title_deed_date','')::date ELSE title_deed_date END,
    owner_name                     = CASE WHEN _patch ? 'owner_name'            THEN nullif(btrim(coalesce(_patch->>'owner_name','')),'') ELSE owner_name END,
    owner_id_number                = CASE WHEN _patch ? 'owner_id_number'       THEN nullif(btrim(coalesce(_patch->>'owner_id_number','')),'') ELSE owner_id_number END,
    land_use_type                  = CASE WHEN _patch ? 'land_use_type'         THEN nullif(btrim(coalesce(_patch->>'land_use_type','')),'') ELSE land_use_type END,
    plot_number                    = CASE WHEN _patch ? 'plot_number'           THEN nullif(btrim(coalesce(_patch->>'plot_number','')),'') ELSE plot_number END,
    block_number                   = CASE WHEN _patch ? 'block_number'          THEN nullif(btrim(coalesce(_patch->>'block_number','')),'') ELSE block_number END,
    plan_number                    = CASE WHEN _patch ? 'plan_number'           THEN nullif(btrim(coalesce(_patch->>'plan_number','')),'') ELSE plan_number END,
    government_notes               = CASE WHEN _patch ? 'government_notes'      THEN nullif(btrim(coalesce(_patch->>'government_notes','')),'') ELSE government_notes END,
    client_user_id = CASE
